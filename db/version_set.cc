...
int FindFile(const InternalKeyComparator& icmp, // internal_key comparator
  ...
  while (left < right) { // 左闭右开区间
    ...
    if (icmp.InternalKeyComparator::Compare(f->largest.Encode(), key) < 0) {
      ...
      left = mid + 1; // 如果 mid 小于 key，那 [0, mid] 都不满足条件
    } else {
...
bool SomeFileOverlapsRange(
  ...
  // 如果各个文件之间无交集，则可以使用二分法来查找
  uint32_t index = 0;
  if (smallest_user_key != nullptr) {
    // Find the earliest possible internal key for smallest_user_key
    InternalKey small(*smallest_user_key, kMaxSequenceNumber,kValueTypeForSeek);
    index = FindFile(icmp, files, small.Encode()); // 在 files 中寻找第一个最大 key 大于等于 smallest_user_key 的文件
  }
  ...
  // 需要确认 files[index].smallest <= largest_user_key 成立。
  ...
  // 前面确认了 files[index].largest >= smallest_user_key，
...
static void SaveValue(void* arg, const Slice& ikey, const Slice& v) {
  ...
  if (!ParseInternalKey(ikey, &parsed_key)) {
    ...
    if (s->ucmp->Compare(parsed_key.user_key, s->user_key) == 0) {
      ...
      if (s->state == kFound) { // 将对应的 value 赋值到 saver 对应成员中
...
void Version::ForEachOverlapping(Slice user_key, Slice internal_key,
  ...
  const Comparator* ucmp = vset_->icmp_.user_comparator(); // 取出用于比较 user_key 的 comparator
  ...
  tmp.reserve(files_[0].size()); // 数目确定，一次性分配，避免后续的重分配导致耗时
  // 顺序搜索 level-0 文件寻找与 user_key 重叠的文件
  for (uint32_t i = 0; i < files_[0].size(); i++) {
    FileMetaData* f = files_[0][i];
    if (ucmp->Compare(user_key, f->smallest.user_key()) >= 0 &&
        ucmp->Compare(user_key, f->largest.user_key()) <= 0) {
      tmp.push_back(f); // 如果 user_key 落在了 f 文件范围内，则将 f 加入 tmp
    }
  }
  if (!tmp.empty()) {
    std::sort(tmp.begin(), tmp.end(), NewestFirst); // 将与 user_key 重叠的文件进行排序，从最新到最旧
    for (uint32_t i = 0; i < tmp.size(); i++) {
      if (!(*func)(arg, 0, tmp[i])) { // 按照文件最新到最旧调用 func
        return;
      }
    }
  }
  ...
  /**
   * 处理其它 levels，因为除了 level-0 其它 level 内部文件都不存在重叠（而且还是有序的），所以本身
   */
  // Search other levels.
  for (int level = 1; level < config::kNumLevels; level++) {
    size_t num_files = files_[level].size();
    if (num_files == 0) continue; // 有的 level 可能为空
    ...
    if (index < num_files) {
      ...
      if (ucmp->Compare(user_key, f->smallest.user_key()) < 0) { // 这个判断确保 user_key 与 f 文件重叠
        // All of "f" is past any data for user_key
      } else {
        if (!(*func)(arg, level, f)) { // 存在重叠，则在该文件上调用 func
          return;
        }
      }
    }
...
Status Version::Get(const ReadOptions& options,
  ...
  for (int level = 0; level < config::kNumLevels; level++) {
    ...
    if (level == 0) {
      ...
      // 找到全部与 user_key 由重叠的文件，然后从最新到最旧顺序进行处理。
      tmp.reserve(num_files); // 已知存储上限，预分配，避免后续重分配消耗性能
      for (uint32_t i = 0; i < num_files; i++) {
        FileMetaData* f = files[i]; // 遍历 vector，检查每个文件是否与 user_key 重叠
        if (ucmp->Compare(user_key, f->smallest.user_key()) >= 0 &&
            ucmp->Compare(user_key, f->largest.user_key()) <= 0) {
          tmp.push_back(f); // 将有重叠的文件加入到临时存储
        }
      }
      if (tmp.empty()) continue; // level-0 没有文件与 user_key 由重叠

      std::sort(tmp.begin(), tmp.end(), NewestFirst); // 按照 file number 对文件进行从最新到最旧排序
      files = &tmp[0]; // 修改指向
      num_files = tmp.size(); // 重叠的文件总个数
    } else {
      // Binary search to find earliest index whose largest key >= ikey.
      // 采用二分查找定位最早的那个满足最大 key >= ikey 的文件索引
      uint32_t index = FindFile(vset_->icmp_, files_[level], ikey);
      if (index >= num_files) { // 没找到
        files = nullptr;
        num_files = 0;
      } else { // 找到了，再比较 user_key，确保 user_key 与该文件范围重叠
        tmp2 = files[index];
        if (ucmp->Compare(user_key, tmp2->smallest.user_key()) < 0) { // 未重叠
    ...
    for (uint32_t i = 0; i < num_files; ++i) {
      if (last_file_read != nullptr && stats->seek_file == nullptr) {
        ...
        // 如果不能查一次文件就找到，需要进行记录下查询的上一个 level 以及文件
...
bool Version::RecordReadSample(Slice internal_key) {
  ...
  State state;
  state.matches = 0;
  // 在每个与 user_key 重叠的文件上面运行上面定义的 Match 方法，
...
void Version::Unref() {
  ...
  --refs_;
  if (refs_ == 0) {
    delete this; // 如果该 version 没有了活跃引用则释放其空间
  }
}
...
bool Version::OverlapInLevel(int level,
                             const Slice* smallest_user_key,
                             const Slice* largest_user_key) {
  // (level > 0) 表示 level-0 以上的 level 内部文件之间才不会由重叠（有序当然是默认成立的）
...
int Version::PickLevelForMemTableOutput(
    ...
    const Slice& largest_user_key) {
  int level = 0;
  // 检查 level-0 某文件是否与 [smallest_user_key, largest_user_key] 有重叠，
  // 如果存在重叠，返回该 0 level；否则进一步检查。
  if (!OverlapInLevel(0, &smallest_user_key, &largest_user_key)) {
    // Push to next level if there is no overlap in next level,
    // and the #bytes overlapping in the level after that are limited.
    InternalKey start(smallest_user_key, kMaxSequenceNumber, kValueTypeForSeek);
    InternalKey limit(largest_user_key, 0, static_cast<ValueType>(0)); // 针对 internal_key，user_key 部分越大越大，序列号越小越大，类型越小越大
    std::vector<FileMetaData*> overlaps;
    while (level < config::kMaxMemCompactLevel) {
      // 检查该 level 的父 level 中是否存在与 [smallest_user_key, largest_user_key] 重叠的文件
      if (OverlapInLevel(level + 1, &smallest_user_key, &largest_user_key)) {
        break; // 如果该 level 的父 level 中存在与 [smallest_user_key, largest_user_key] 重叠的文件则跳出循环，返回该 level
      }
      // 如果该 level 的父 level 中不存在与 [smallest_user_key, largest_user_key] 重叠的文件，
      // 则检查该 level 的祖父 level 是否存在过多与 [smallest_user_key, largest_user_key] 重叠的文件。
      // 如果文件数超过阈值，则表示该 level 需要进行压实了。
      if (level + 2 < config::kNumLevels) {
        // Check that file does not overlap too many grandparent bytes.
        // 获取该 level 的祖父 level 中与 [smallest_user_key, largest_user_key] 有重叠的全部文件，
        // 并保存到 overlaps。
        GetOverlappingInputs(level + 2, &start, &limit, &overlaps);
        // 计算祖父 level 中与 [smallest_user_key, largest_user_key] 有重叠的全部文件的总大小
        const int64_t sum = TotalFileSize(overlaps);
        // 如果大小超过了阈值，表示祖父 level 存在过多与 [smallest_user_key, largest_user_key] 重叠的文件，则跳出循环返回该 level
...
//
// 将 "level" 中与 [begin,end] 有重叠的全部文件保存到 "*inputs"
